...
// Setup -- This part sets up the scene's overall camera matrix, projection matrix, and lights:
        ...
        //program_state.lights = [new Light(light_position, color(1, 1, 1, 1), 10 ** (t % 3))];
                ...
                const sun_radius = 1 + 2 * Math.abs(Math.sin(t / 10));  
        

                // Calculate the sun's color
                const sun_color = color(1, Math.abs(Math.sin(t / 10)), Math.abs(Math.sin(t / 10)), 1);
                
                // Create a point light source with the same color as the sun and a size parameter equal to the current sun radius
                //const light_position = model_transform.times(vec4(0,0,0,1)); // Assuming the light is at the center of the sun
                program_state.lights = [new Light(light_position, sun_color, sun_radius)];
