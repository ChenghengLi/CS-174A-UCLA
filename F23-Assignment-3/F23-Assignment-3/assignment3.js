...
export class Assignment3 extends Scene {
    constructor() {
        ...
        this.materials = {
            ...
            sun: new Material(new defs.Phong_Shader(),{ambient: 1, diffusivity: 1, specularity: 0, color: hex_color("#ffffff")}),
            planet1: new Material(new defs.Phong_Shader(), {ambient: 0, diffusivity: 1, specularity: 0, color: hex_color("#808080")}),
            planet2_gouraud: new Material(new Gouraud_Shader(), {ambient: 0, diffusivity: .2, specularity: 1, color: hex_color("#80FFFF")}),
            planet2_phong: new Material(new defs.Phong_Shader(),{ambient: 0, diffusivity: .2,  specularity: 1, color: hex_color("#80FFFF")}),
            planet3: new Material(new defs.Phong_Shader(), {ambient: 0, diffusivity: 1, specularity: 1, color: hex_color("#B08040")}),
            planet4: new Material(new defs.Phong_Shader(), {ambient: 0, diffusivity: 0.6, specularity: 1, color: hex_color("#ADD8E6")}),
            moon: new Material(new defs.Phong_Shader(), {ambient: 0.4, diffusivity: 0.6, specularity: 0.6, color: hex_color("#ffffff")}),
            ring: new Material(new Ring_Shader()),
        }
    ...
    display(context, program_state) {
        ...
        // Implement part 2: Add a point light source with the same color as the sun and a size parameter equal to the current sun radius


         // TODO:  Fill in matrix operations and drawing code to draw the solar system scene (Requirements 3 and 4)
        for (let i = 0; i < 4; i++) {
            // Set the orbit radius and rotation speed
            let planet_transform = Mat4.identity();
            let orbit_radius = 5 + i * 3;
            let orbital_speed = 1/ (i + 1);
            // Rotate the planet around the sun
            planet_transform = planet_transform
                                .times(Mat4.rotation(t*orbital_speed , 0, 1, 0))
                                .times(Mat4.translation(orbit_radius, 0, 0));
        
            // Add a moon for the fourth planet
            if (i === 3) {
                let moon_transform = planet_transform.times(Mat4.rotation(t, 0, 1, 0))
            ...
            if (i == 2) {
                planet_transform = planet_transform.times(Mat4.rotation(Math.sin(t), 1, 0, 0));
                var ring_transform = planet_transform.times(Mat4.scale(3.5, 3.5, 0.1));                                                                                     
            ...
            if(i == 1){
                if(Math.floor(t) % 2 == 1){
                    ...
                    this.shapes.planet2.draw(context, program_state, planet_transform, this.materials.planet2_gouraud);
                }
                else{
                    // Phong shading every even second
                    this.shapes.planet2.draw(context, program_state, planet_transform, this.materials.planet2_phong);
                }
            }else{
                let planet_material = this.materials[`planet${i+1}`];
                this.shapes[`planet${i+1}`].draw(context, program_state, planet_transform, planet_material);
            }

            // Store the camera matrix for each planet
            this[`planet_${i+1}`] = Mat4.inverse(planet_transform.times(Mat4.translation(0, 0, 5)));
        }
